...
def importForm(month, month_number, year, test):
    ...
    for index, row in parentFormsIds.iterrows():
        ...
        AT5_01_RM_NA_BB = int(df.loc[df['age_gender'] == 'NINAS'].iloc[0].value_a)
        AT5_01_RM_NN_BB = int(df.loc[df['age_gender'] == 'NINOS'].iloc[0].value_a)

        dfCa = osmosys.osmosys.getCAValues(year=year, month=month, orgOsmosys=orgAcron,
                                           indicatorsIdsOmosys=indicatorIdsOsmosys, cantonCode=cantonCode)
        AT5_01_CA_NA_BB = int(dfCa.loc[df['age_gender'] == 'NINAS'].iloc[0].value_a)
        AT5_01_CA_NN_BB = int(dfCa.loc[df['age_gender'] == 'NINOS'].iloc[0].value_a)
        ...
        subform = model.modelAI.SubFormAT5_01(
            ...
            AT5_01_RM_NA_BB=AT5_01_RM_NA_BB,
            AT5_01_RM_NN_BB=AT5_01_RM_NN_BB,
            AT5_01_CA_NA_BB=AT5_01_CA_NA_BB,
            AT5_01_CA_NN_BB=AT5_01_CA_NN_BB,
