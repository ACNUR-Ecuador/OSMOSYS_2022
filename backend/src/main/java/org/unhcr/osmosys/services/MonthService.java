    ...
    public void blockMonthsByYearAndoMonth(int year, MonthEnum month, boolean block) throws GeneralAppException {
        ...
        if (currentMonth == 6 || currentMonth == 12) {
            ...
            for (MonthEnum monthEnum : monthsOfSemester) {
                LOGGER.debug("monthEnum >"+monthEnum);
                LOGGER.debug("year >"+year);
                LOGGER.debug("!block >"+!block);
                LOGGER.debug("Frecuency.SEMESTRAL >"+Frecuency.SEMESTRAL);
        ...
        if (currentMonth == 12) {
            ...
            for (MonthEnum monthEnum : MonthEnum.values()) {
                ...
                LOGGER.debug("!block >"+!block);
                LOGGER.debug("Frecuency.SEMESTRAL >"+Frecuency.SEMESTRAL);
                List<Month> unlockedMonthsAnnual = this.monthDao.getActiveMonthsAndMonthAndYearAndBlockingStatusAndFrecuency(monthEnum, year, !block, Frecuency.ANUAL);
