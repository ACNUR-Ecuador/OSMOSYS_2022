...
SELECT
    ...
    COALESCE(ie.assigned_user_backup_id, 0) AS assigned_user_backup_id,
    COALESCE(ie.performance_indicator_id, 0) AS indicator_id,
    COALESCE(i.statement_id, 0) AS statement_id,
    COALESCE(ie.project_statement_id, 0) AS project_statement_id,
    ie.indicator_type,
    COALESCE(pr.organization_id, 1) AS organization_id,
    COALESCE(pr.focal_point_id, ie.supervisor_user_id) AS supervisor_id,
    COALESCE(ie.reporting_office_id, 0) AS office_id,
    COALESCE(ie.project_id, 0) AS project_id,
    (q.year || '-') || mo.month_year_order AS month_year_id,
    iv.dissagregation_type,
    COALESCE(iv.age_type, 'NO APLICA') AS age_type,
    COALESCE(iv.age_primary_education_type, 'NO APLICA') AS age_primary_education_type,
    COALESCE(iv.age_tertiary_education_type, 'NO APLICA') AS age_tertiary_education_type,
    COALESCE(iv.gender_type, 'NO APLICA') AS gender_type,
    COALESCE(iv.country_of_origin, 'NO APLICA') AS country_of_origin,
    COALESCE(iv.population_type, 'NO APLICA') AS population_type,
    COALESCE(iv.diversity_type, 'NO APLICA') AS diversity_type,
    COALESCE(iv.canton_id, 0) AS canton_id,
    NULL as custom_dissagregation_id,

    COALESCE(COALESCE(can.office_id,ie.reporting_office_id),usetfppr.office_id) as responsable_office_id,
    ...
    iv.value
FROM osmosys.periods per
         JOIN osmosys.indicator_executions ie ON ie.period_id = per.id  AND ie.state = 'ACTIVO'
         JOIN osmosys.quarters q ON q.indicator_execution_id = ie.id AND q.state= 'ACTIVO'
         JOIN osmosys.months mo ON mo.quarter_id = q.id AND mo.state = 'ACTIVO'
         JOIN osmosys.indicator_values iv ON mo.id = iv.month_id AND iv.state = 'ACTIVO'
...
FROM
    osmosys.periods per
        INNER JOIN osmosys.indicator_executions ie ON ie.state = 'ACTIVO' AND ie.period_id = per.id
        INNER JOIN osmosys.quarters q ON  q.state = 'ACTIVO' AND q.indicator_execution_id = ie.id
        INNER JOIN osmosys.months mo ON  mo.state = 'ACTIVO' AND mo.quarter_id = q.id
        INNER JOIN osmosys.indicator_values_custom_dissagregation iv ON iv.month_id=mo."id" AND iv.state = 'ACTIVO'
        INNER JOIN osmosys.custom_dissagregation_options cdo on iv.custom_dissagregation_option=cdo."id"
